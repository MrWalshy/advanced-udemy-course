...
function hasDuplicates(arr){
    // Set removes duplicates, if the Sets size is not equal to the arrays length - there is duplicates
...
function countPairs(arr, num){
    ...
    for(let value of arr){
        cache.delete(value);
        if(cache.has(num - value)){
            // countPairs([8,2,6,4,10,0],10) // 3
            // If num - value = x and cache contains x, add 1 to matching count
            count++;
        }
    }
    return count;
